...
import { createUser, deleteUser, updateUser } from "@/Controller/User/CRUD";
...
export async function POST(req) {
  ...
  if (!WEBHOOK_SECRET) {
    throw new Error(
      "Please add CLERK_WEBHOOK_SECRET from Clerk Dashboard to .evn or .env.local"
    );
  }
  ...
  if (eventType == "user.deleted") {
    await dbConnect();
    const res = await deleteUser({ body: userData });
    return new Response(res.body, res.status);
  }
